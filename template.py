...
def chat_with_system_prompt(
    ...
    # Gọi API với mảng messages gồm 2 role
    response = client.chat.completions.create(
        model=model,
        messages=[
            {"role": "system", "content": system_prompt},
...
def count_tokens(text: str, model: str = OPENAI_MODEL) -> int:
    ...
    try:
        ...
        # Lấy bộ mã hóa tương ứng với model
        enc = tiktoken.encoding_for_model(model)
        
        # Mã hóa text và đếm độ dài mảng token trả về
        return len(enc.encode(text))
        
    except Exception:
        # Fallback (dự phòng): Nếu bị lỗi mạng hoặc model lạ (như gemini)
        # mà tiktoken không nhận diện được, ta ước lượng thô.
        # Trung bình 1 token tương đương khoảng 4 ký tự.
...
def estimate_cost(prompt: str, response: str, model: str = OPENAI_MODEL) -> dict:
    ...
    completion_tokens = count_tokens(text=response, model=model)
...
def streaming_chatbot() -> None:
    ...
    while True:
        ...
        # Chỉ giữ 4 lượt hội thoại gần nhất (8 message)
        history = history[-8:]
